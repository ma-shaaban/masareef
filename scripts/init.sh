...
# Substitutes the ma-shaaban/masareef placeholders in deploy/ + catalog-info.yaml
...
grep -rq "masareef" deploy catalog-info.yaml || { echo "no placeholders left — already initialized?"; exit 1; }
...
for f in deploy/base/*.yaml deploy/staging/*.yaml deploy/prod/*.yaml deploy/preview/*.yaml catalog-info.yaml; do
  tmp="$(mktemp)"
  sed -e "s/ma-shaaban/$USER_ARG/g" -e "s/masareef/$APP_ARG/g" \
